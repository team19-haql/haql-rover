...
namespace bodenbot
{
    ...
    hardware_interface::CallbackReturn SerialController::on_init(
...
#define DEBUG_PARAM(message, name) RCLCPP_INFO(\
    rclcpp::get_logger("SerialController"), message,\
...
#undef DEBUG_PARAM
        ...
        for (hardware_interface::ComponentInfo& joint : info_.joints)
        {
            ...
            RCLCPP_INFO(
                rclcpp::get_logger("SerialController"),
                "Found joint '%s' with id '%s' and reversed '%s'", joint.name.c_str(),
                joint.parameters["id"].c_str(), joint.parameters["reversed"].c_str());
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("SerialController"), "Activating ...please wait...");
        ...
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("SerialController"), "Deactivating ...please wait...");
        ...
        RCLCPP_INFO(rclcpp::get_logger("SerialController"), "Successfully deactivated!");


        return hardware_interface::CallbackReturn::SUCCESS;
    }

    hardware_interface::return_type SerialController::read(
    ...
    hardware_interface::return_type bodenbot::SerialController::write(
        const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/)
    {
        for (auto i = 0u; i < info_.joints.size(); i++) {
            int id = motor_id_[i];
...
PLUGINLIB_EXPORT_CLASS(
    bodenbot::SerialController, hardware_interface::SystemInterface)
