...
def generate_launch_description():
    ...
    valley_config_file = LaunchConfiguration('valley_config')
    ...
    declare_valley_config_file_cmd = DeclareLaunchArgument(
        'valley_config',
        default_value=os.path.join(
            traverse_layer_dir, 'config', 'valley_image.yml'),
        description='Full path to the valley image config file to use')
    ...
    image_publisher_node = Node(
        ...
        output='screen',
        parameters=[{
            'image_path': os.path.join(traverse_layer_dir, 'data', 'valley.png'),
            'topic': 'image',
        }],
    )
    ...
    ld.add_action(declare_valley_config_file_cmd)
